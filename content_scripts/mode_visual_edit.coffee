...
class SelectionManipulator
  ...
  getDirection: ->
    ...
    if element and DomUtils.isEditable(element) and not element.isContentEditable
      ...
      documentPosition = @selection.focusNode.compareDocumentPosition @selection.anchorNode
      if documentPosition == 0
        if @selection.focusOffset < @selection.anchorOffset then backward else forward
      else if documentPosition & Node.DOCUMENT_POSITION_PRECEDING
        forward
      else # documentPosition & Node.DOCUMENT_POSITION_FOLLOWING
        backward
...
class Movement extends CountPrefix
  ...
  runMovements: (movements...) ->
    for movement in movements
      return false unless @selectionManipulator.selectionChanged => @runMovement movement
    true
  ...
  runMovementKeyChar: (args...) ->
    @selectionManipulator.protectClipboard => @handleMovementKeyChar args...
        ...
        doFind = (count, backwards) =>
          ...
          for [0...count] by 1
            unless FindMode.execute null, {colorSelection: false, backwards}
              @selectionManipulator.setSelectionRange initialRange
  ...
  selectLexicalEntity: (entity, count = 1) ->
    ...
    switch entity
      when word
        if @selectionManipulator.nextCharacterIsWordCharacter()
      ...
      when paragraph
        ...
        char = @selectionManipulator.getNextBackwardCharacter()
        while char? and char != "\n"
          return unless @runMovements [ backward, character ], [ backward, lineboundary ]
          char = @selectionManipulator.getNextBackwardCharacter()
        @selectionManipulator.collapseSelectionToFocus()
        char = @selectionManipulator.getNextForwardCharacter()
        while char? and char != "\n"
  ...
  scrollIntoView: ->
    @selectionManipulator.protectClipboard =>
      if @element and DomUtils.isEditable @element
        if @element.clientHeight < @element.scrollHeight
          if @element.isContentEditable
            ...
            position = if @selectionManipulator.getDirection() == backward then @element.selectionStart else @element.selectionEnd
...
class VisualMode extends Movement
  constructor: (options = {}) ->
    ...
    unless @options.oneMovementOnly
      ...
      @commands.c = -> @selectionManipulator.collapseSelectionToFocus(); @changeMode CaretMode
  ...
  exit: (event, target) ->
    ...
    if @yankedText?
      unless @options.noCopyToClipboard
        ...
        @selectionManipulator.copy @yankedText, true
  ...
  selectLine: (count) ->
    @reverseSelection() if @selectionManipulator.getDirection() == forward
    ...
    @runMovement forward, character if @selectionManipulator.getNextForwardCharacter() == "\n"
...
class VisualLineMode extends VisualMode
  ...
  extendSelection: ->
    initialDirection = @selectionManipulator.getDirection()
    ...
    switch @selectionManipulator.selection.type
      when "None"
        ...
        if @selectionManipulator.selection.type == "None"
          ...
          return
      when "Range"
        @selectionManipulator.collapseSelectionToAnchor()
...
class EditMode extends Movement
  ...
  pasteClipboard: (direction) ->
    @selectionManipulator.paste (text) =>
      if text
